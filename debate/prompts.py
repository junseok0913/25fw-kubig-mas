"""Prompt templates for the ticker debate pipeline.

This module ports the original multi-agent debate prompt style from
`stock-morning-main/multiagent/prompts.py`, adapted for this repo:
- Uses news + SEC filings + chart/price
- Tools are `get_news_content(pks=[...])`, `get_ohlcv(...)`, `get_sec_filing_content(...)`
- Every expert turn must output JSON: {"text": "...", "sources": [...]}
"""
...
# ============================================================
# 1) Expert persona prompts (ported)
# ============================================================
...
**도구 사용:**
- 뉴스 헤드라인의 상세 내용이 필요하면 `get_news_content(pks=[...])` 도구를 호출하세요
  - pks는 위 뉴스 목록의 pk를 그대로 사용하세요
- 가격/차트가 더 필요하면 `get_ohlcv(ticker=..., start_date=..., end_date=...)`를 호출하세요
- SEC 공시 원문이 필요하면 먼저 `get_sec_filing_content(ticker=..., accession_numbers=[...])`로 index(목차/요약)를 확인하세요
- 그 다음 필요한 페이지만 `page=1,2,3...`으로 나눠서 content를 조회하세요
