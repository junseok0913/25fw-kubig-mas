"""Calendar prefetcher (TradingEconomics)."""
...
def _cal_window_for_anchor_date(anchor_date_et: date, days_back: int, days_forward: int) -> tuple[datetime, datetime]:
    tz = ZoneInfo(CAL_WINDOW_TZ)
    start = datetime.combine(anchor_date_et - timedelta(days=days_back), time.min).replace(tzinfo=tz)
    end = datetime.combine(anchor_date_et + timedelta(days=days_forward), time.max).replace(tzinfo=tz)
    return start, end
...
def _cal_write_calendar_csv(events: Sequence[Dict[str, Any]], cache_dir: Path) -> None:
    cache_dir.mkdir(parents=True, exist_ok=True)
    csv_path = cache_dir / "calendar.csv"
...
def _cal_write_calendar_json(meta: Dict[str, Any], events: Sequence[Dict[str, Any]], cache_dir: Path) -> None:
    cache_dir.mkdir(parents=True, exist_ok=True)
    payload = {**meta, "events": list(events)}
    json_path = cache_dir / "calendar.json"
...
def prefetch_calendar(
    ...
    """Scrape TradingEconomics calendar and write to cache."""
    _load_env()
    cache_dir.mkdir(parents=True, exist_ok=True)

    start_et, end_et = _cal_window_for_anchor_date(today, days_back=days_back, days_forward=days_forward)
    ...
    _cal_write_calendar_csv(merged, cache_dir)
    _cal_write_calendar_json(meta, merged, cache_dir)

    logger.info("calendar prefetch 완료: %d events -> %s", len(merged), cache_dir)
    return {"count": len(merged), "csv": str(cache_dir / "calendar.csv"), "json": str(cache_dir / "calendar.json")}
