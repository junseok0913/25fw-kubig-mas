...
- 오케스트레이터 Stage 실행:
  - `--stage 3`에서 ClosingAgent가 실행됩니다.
- 단독 실행:
  - 입력 스크립트가 주어지지 않으면 `temp/ticker_pipeline.json`을 우선 읽고, 없으면 `temp/theme.json`을 읽어 대본을 로드합니다.
...
| --- | --- | --- | --- | --- |
| 티커 파이프라인 산출물 | `temp/ticker_pipeline.json` | Orchestrator(TickerPipeline) | ClosingAgent | standalone 모드에서 우선 사용 |
| 테마 결과 산출물 | `temp/theme.json` | ThemeAgent | ClosingAgent | ticker 미사용 시 fallback |
...
| --- | --- | --- | --- |
| `load_scripts_from_temp` | scripts가 비어있으면 `temp/ticker_pipeline.json` 우선, 없으면 `temp/theme.json`에서 로드 | `scripts`, (fallback) `temp/*` | `date/scripts` 채움 |
...
2) standalone 모드: `scripts`가 비어 있으면 `temp/ticker_pipeline.json`을 우선 사용하고, 없으면 `temp/theme.json`에서 로드합니다.
...
- Via orchestrator: `python orchestrator.py 20251222 --agent closing`
  ...
  - `temp/ticker_pipeline.json`이 있으면 우선 사용하고, 없으면 `temp/theme.json`을 사용합니다.
  - 둘 다 없으면 실패합니다.
...
- `temp/ticker_pipeline.json`/`temp/theme.json` 누락:
